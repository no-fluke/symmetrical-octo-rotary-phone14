...
- Generates a .txt file with:
  ...
  • Practice sheet links with names
...
The bot generates a structured text file with all the links."""
    ...
    def extract_practice_sheets(self, class_data, teacher_name):
        """Extract practice sheets from class data"""
        practice_sheets = []
        
        # Try different possible keys for practice sheets
        possible_keys = ['practiceSheets', 'practice_sheets', 'practice', 'sheets', 'assignments']
        
        for key in possible_keys:
            if key in class_data:
                sheets = class_data.get(key, [])
                if isinstance(sheets, list):
                    for sheet in sheets:
                        sheet_url = sheet.get('url') or sheet.get('link')
                        sheet_name = sheet.get('name') or sheet.get('title') or 'Practice Sheet'
                        if sheet_url and sheet_url.startswith(('http://', 'https://')):
        ...
        # Also check if there are practice sheets in nested structures
        if not practice_sheets:
            # Check in materials or resources
            materials = class_data.get('materials', {})
            if isinstance(materials, dict):
                for material_type, material_list in materials.items():
                    if material_type.lower() in ['practice', 'sheet', 'assignment', 'exercise']:
                        if isinstance(material_list, list):
                            for material in material_list:
                                sheet_url = material.get('url') or material.get('link')
                                sheet_name = material.get('name') or material.get('title') or f'Practice Sheet {material_type}'
                                if sheet_url and sheet_url.startswith(('http://', 'https://')):
                                    practice_sheets.append({
                                        'name': sheet_name,
                                        'url': sheet_url,
    ...
    def generate_formatted_text_file(self, course_info, classes_data, preferred_quality):
        ...
        # First, collect all video classes and their materials
        all_classes = []
        ...
        for topic in classes_data:
            ...
            for class_data in topic_classes:
                ...
                # Get practice sheets for this class
                practice_sheets = self.extract_practice_sheets(class_data, teacher_name)
                
                if video_url or pdfs or practice_sheets:
                    all_classes.append({
        ...
        for class_info in all_classes:
            ...
            # Add class header
            if class_info['class_title']:
                lines.append(f"📹 Class {class_info['class_number']}: {class_info['class_title']}")
            else:
                lines.append(f"📹 Class {class_info['class_number']}")
            
            # Add teacher info
            lines.append(f"👨‍🏫 Teacher: {class_info['teacher_name']}")
            
            # Add video URL if available
            if class_info['video_url']:
            ...
            # Add practice sheets if available
            if class_info['practice_sheets']:
                lines.append("")
                lines.append("📝 Practice Sheets:")
                for sheet in class_info['practice_sheets']:
                    lines.append(f"  • {sheet['name']}: {sheet['url']}")
            
            lines.append("")  # Empty line between classes
            lines.append("")  # Additional empty line for spacing
        
        # Add summary at the end
        lines.append("=" * 60)
        total_classes = len(all_classes)
        total_pdfs = sum(len(c['pdfs']) for c in all_classes)
        total_sheets = sum(len(c['practice_sheets']) for c in all_classes)
        
        lines.append(f"📊 SUMMARY:")
        lines.append(f"  Total Classes: {total_classes}")
        lines.append(f"  Total PDFs: {total_pdfs}")
        lines.append(f"  Total Practice Sheets: {total_sheets}")
        lines.append("")
        lines.append("✅ All materials fetched successfully!")
